...
# Load the trained model pipeline once at start-up (not per request)
model = joblib.load("superkart_model.joblib")

# Define a route for the home page - also serves as a health check
@superkart_api.get('/')
def home():
...
def predict_sales():
    ...
    # Extract the 10 features the model pipeline expects
    sample = {
    ...
    # so raw human-readable values can be passed straight through.
...
# Run the Flask app in debug mode (local development only;
# in the container Gunicorn serves the app instead)
